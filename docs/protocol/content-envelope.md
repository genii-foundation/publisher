...
The publication manifest declares extensions in resolution order. Each declaration has a nonempty ordered list of unique grants from the closed extension capability vocabulary. The host resolves exactly one matching package version for each declaration and injects the manifest's exact grants in the same extension and capability order. The compiler neither imports nor executes extension packages. It rejects missing, extra, reordered, mismatched, unknown, or duplicate resolutions and grants.

Typed content payloads let those resolved extensions contribute canonical JSON data without mutating the compiler. Payloads are sorted by ID. Every listed `sourcePath` must identify an injected `extension` source owned by the payload's extension, and one source path cannot belong to two extensions. The envelope derives each extension's ordered `payloadIds` and verifies every payload hash. The exact ordered capability grants, extension source provenance, payload data, and producer identity participate in content and build identity.
