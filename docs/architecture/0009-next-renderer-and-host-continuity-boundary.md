...
The fixed footer is renderer owned and appears on every publication page, the engine-owned not-found page, attributed static framework 404 and 500 pages, and the hydrated App Router error boundaries. It renders Exhibit B values from engine constants rather than caller fields, displays and links the required GENII Publisher attribution, and exposes the publication source URL.

The package exposes client-safe error components and a runtime identity factory only through `@genii-foundation/publisher-next/client`. The host supplies `app/error.tsx`, `app/global-error.tsx`, `pages/404.tsx`, `pages/500.tsx`, `pages/_error.tsx`, `pages/_app.tsx`, and `pages/_document.tsx`. Those files receive one validated public identity generated from the application's already validated reader and theme. The factory rejects changed attribution, missing or unsafe source URLs, malformed home routes, and invalid theme tokens. These client files do not import the reader artifact or server application.
