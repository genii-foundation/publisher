...
Every section adapter input declares a `readerLocation`. A `work` location selects the unfragmented work route and may therefore belong to only one section. A `route` location selects one named section address exactly. A `none` location is valid only for a nonnavigable section. The compiler resolves that declaration to `readerAddress`; it never chooses a preferred address from route names, insertion order, or title conventions.

Every content block has two identities. `id` identifies the compiler projection. `anchor` is the portable fragment component used by readers and bookmarks when the section has a reader address. A block on an unanchored reader address resolves to `#<block-anchor>`. A block on an anchored reader address resolves to `#<section-anchor>-<block-anchor>`. A block in an unlocated nonnavigable section has no public destination. The compiler rejects collisions across section and block addresses. Block content hashes remain content-only so a renamed anchor does not destroy relocation evidence. Section, work, content, and build hashes still change because section identity includes every block anchor.
...
Asset hrefs are plain concrete origin-relative route paths. They are not `ContentAddress` values and cannot carry a query, fragment, route template, or absolute URL. Each browser-equivalent section `ContentAddress`, including its optional decoded anchor, has one section owner across the publication. Every resolved reader address and derived block address also has one publication-wide owner. Section reader locations may use structured anchors, but every non-active address must use an active server route as its base path. An unanchored section address must use its own work route or an active section route owned by the same section. Home, Updates, collection, and unrelated work routes cannot become implicit section hosts. A future composed-page feature must declare that relationship explicitly. Resolved links may use one validated fragment or an absolute credential-free HTTP URL.
...
The current `compileMarkdownWork` helper is the neutral CommonMark adapter primitive. It requires explicit work and root-section IDs, emits one section, and derives distinct content-addressed block projection IDs and public anchors. With no section route, its explicit reader location is the work route. With a supplied route, that named address becomes the reader location. It does not infer a multi-section hierarchy, title-derived durable IDs, continuity history, or publication-specific heading rules. A future adapter package may build richer work input around this primitive. If authoring source lacks stable section identities, a separate migration command must propose durable declarations for human review before they become a published contract.
