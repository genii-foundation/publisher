...
export function immutableSnapshot<T>(value: T): T {
  ...
  while (pending.length > 0) {
    ...
    const arraySource = Array.isArray(current.source);
    const arrayLength = arraySource
      ? (current.source as unknown[]).length
      : 0;
    let arrayIndexCount = 0;
    for (const ownKey of Reflect.ownKeys(current.source)) {
      if (arraySource && ownKey === "length") {
        continue;
      }
      if (typeof ownKey !== "string") {
        throw new TypeError(
          "Immutable snapshots reject symbol properties.",
        );
      }
      const key = ownKey;
      ...
      if (arraySource) {
        ...
        if (
          ...
          throw new TypeError(
            "Immutable snapshots reject non-index array properties.",
          );
        }
        arrayIndexCount += 1;
      }
    ...
    if (
      arraySource &&
      arrayIndexCount !== arrayLength
    ) {
      throw new TypeError("Immutable snapshots reject sparse arrays.");
    }
  }
