...
`@genii-foundation/publisher-reader` bundles its CommonMark parser so a
published reader version always executes the exact parser closure recorded
in the engine lockfile. The bundled runtime contains the following
...
These packages remain subject to their own license terms. They are not GENII Publisher Original Code.
Each bundled package retains its own license file. The direct parser license is also reproduced in
`third-party-licenses/mdast-util-from-markdown-LICENSE-MIT.txt`.
