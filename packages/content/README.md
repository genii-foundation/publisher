...
- `validateAudioPublicationGuard` refuses changed spoken content unless every current narrator publishes the exact current clip backed by immutable evidence for the publication, Reader build, spoken text, object URL, format, byte sizes, and duration. It also binds both reviewed catalog hashes and refuses narrator removal without reading Git, storage, or author files.
- `createSpokenInputIdentity` trims the spoken title, collapses presentation-free body whitespace, returns the exact spoken text and its full SHA-256 identity, and rejects unsafe or ambiguous Unicode input. It does not parse Markdown or reinterpret provider-owned audio version tokens.
