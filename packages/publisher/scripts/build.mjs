...
const internalPackageManifestPaths = Object.freeze([
  join(packageRoot, "..", "content", "package.json"),
  join(packageRoot, "..", "..", "schemas", "package.json"),
]);
...
const [packageManifestText, sourceNotice, ...internalManifestTexts] =
  await Promise.all([
    readFile(packageManifestPath, "utf8"),
    readFile(sourceNoticePath, "utf8"),
    ...internalPackageManifestPaths.map((path) => readFile(path, "utf8")),
  ]);
const packageManifest = JSON.parse(packageManifestText);
const internalManifests = internalManifestTexts.map((text) =>
  JSON.parse(text),
);
...
if (typeof expectedTypeScriptVersion !== "string") {
  throw new Error(
    "packages/publisher/package.json must pin a TypeScript development dependency.",
  );
}

for (const internalManifest of internalManifests) {
  if (
    packageManifest.dependencies?.[internalManifest.name] !==
    internalManifest.version
  ) {
    throw new Error(
      `Publisher must depend on exact ${internalManifest.name} version ${internalManifest.version}.`,
    );
  }
}
...
const builtEntry = await import(
  new URL(
    `../dist/index.js?version=${encodeURIComponent(packageManifest.version)}`,
    import.meta.url,
  )
);
if (builtEntry.PUBLISHER_VERSION !== packageManifest.version) {
  throw new Error(
    `Publisher constant ${builtEntry.PUBLISHER_VERSION ?? "(missing)"} does not match package version ${packageManifest.version}.`,
  );
}
